...
folder_paths = ['/mnt/cgshare/DMT/BraTS2020_TrainingData/t1', '/mnt/cgshare/DMT/BraTS2020_TrainingData/t2', '/mnt/cgshare/DMT/BraTS2020_TrainingData/flair']
...
def zxtrain(dataloader, epoch):
    ...
    TMC.load_state_dict(torch.load('T1_TMC.pt'))
    DEA_AE.load_state_dict(torch.load('T1_DEA_AE_1_1_0.3.pt'))
    Generator.load_state_dict(torch.load('T1_DEA_DE_1_1_0.3.pt'))
    level_1_mix.load_state_dict(torch.load('T1_l1_mix_1_1_0.3.pt'))
    level_2_mix.load_state_dict(torch.load('T1_l2_mix_1_1_0.3.pt'))
    ...
    for num_epoch in tqdm(range(epoch)):
        ...
        for num_batch in tqdm(range(len(dataloader[0])), ncols=100, position=0, leave=True):
            t1, _ = next(iter(dataloader[0]))
            t2, _ = next(iter(dataloader[1]))
            flair, _ = next(iter(dataloader[2]))
            t1 = t1.to(device)
            t2 = t2.to(device)
            ...
            level_1_mix_feature = level_1_mix(flair_feature1, t2_feature1)
            level_2_mix_feature = level_2_mix(flair_feature2, t2_feature2)
            fake_t1 = Generator(level_1_mix_feature, level_2_mix_feature)

            fake_t1_copy = fake_t1.detach().clone()

            for D_epoch in range(TMC_epoch):

                fake_dis_label, fake_tmc_loss = TMC(fake_t1_copy, fake_label, num_epoch)
            ...
            t1_mse_loss = L1Loss(fake_t1, t1)
            com_mse_loss = com_mse_alpha * flair_mse_loss + com_mse_alpha * t2_mse_loss
            # + com_mse_alpha * flair_mse_loss
            mse_loss = com_mse_loss + t1_mse_loss
            ...
            G_loss = mse_loss + E_alpha * e_loss + D_alpha * dis_loss
            ...
            psnr, ssim, lpips = getscore(fake_t1.detach(), flair)
        ...
        if num_epoch % 5 == 0 and num_epoch != 0:
            ...
            score_list.to_csv('T1_score_list_1_1_0.3after400.csv', index=False)
            loss_list = {'Dis_loss': Discriminator_loss, 'Gen_loss': Generator_loss, 'rec_loss': mse_loss_list, 'E_loss': E_loss_list, 'dis_loss': dis_loss_list}
            loss_list = pd.DataFrame(loss_list)
            loss_list.to_csv('T1loss_list_1_1_0.3after400.csv', index=False)

        if num_epoch % 50 == 0 and num_epoch != 0:
            torch.save(DEA_AE.state_dict(), f'T1_DEA_AE_1_1_0.3.pt')
            torch.save(Generator.state_dict(), f'T1_DEA_DE_1_1_0.3.pt')
            torch.save(level_1_mix.state_dict(), f'T1_l1_mix_1_1_0.3.pt')
            torch.save(level_2_mix.state_dict(), f'T1_l2_mix_1_1_0.3.pt')
            torch.save(TMC.state_dict(), f'T1_TMC.pt')
