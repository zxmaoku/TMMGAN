...
def zxtrain(dataloader, epoch):
    ...
    for num_epoch in tqdm(range(epoch)):
        ...
        for num_batch in tqdm(range(len(dataloader[0])), ncols=100, position=0, leave=True):
            ...
            level_1_mix_feature = level_1_mix(t1_feature1, t2_feature1)
            level_2_mix_feature = level_2_mix(t1_feature2, t2_feature2)
            # flair_hat, flair_feature1, flair_feature2 = DEA_AE(flair)
            fake_flair = Generator(level_1_mix_feature, level_2_mix_feature)

            fake_flair_copy = fake_flair.detach().clone()

            for D_epoch in range(TMC_epoch):

                fake_dis_label, fake_tmc_loss = TMC(fake_flair_copy, fake_label, num_epoch)
            ...
            fake_mse_loss = L1Loss(fake_flair, flair)
            com_mse_loss = com_mse_alpha * t1_mse_loss + com_mse_alpha * t2_mse_loss
            # + com_mse_alpha * flair_mse_loss
            mse_loss = com_mse_loss + fake_mse_loss
            ...
            psnr, ssim, lpips = getscore(fake_flair.detach(), flair)
        ...
        if num_epoch % 10 == 0 and num_epoch != 0:
            ...
            score_list.to_csv('FLAIR_score_list_1_1_0.3after1400.csv', index=False)
            ...
            loss_list.to_csv('FLAIR_loss_list_1_1_0.3after1400.csv', index=False)

        if num_epoch % 100 == 0 and num_epoch != 0:
